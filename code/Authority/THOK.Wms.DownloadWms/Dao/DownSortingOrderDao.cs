...
namespace THOK.WMS.DownloadWms.Dao
{
    public class DownSortingOrderDao : BaseDao
   {
       ...
       public DataTable GetSortingOrder(string orderid)
       {
           ...
           switch (dbTypeName)
           {
               case "mdjyc-db2"://牡丹江烟草db2
                   sql = string.Format(@"SELECT a.*,b.DIST_BILL_ID,b.DELIVERYMAN_CODE,b.DELIVERYMAN_NAME,SUBSTR(A.ORDER_ID,3,12) AS ORDERID FROM V_WMS_SORT_ORDER A
       ...
       {           
           ...
           {
               case "mdjyc-db2"://牡丹江烟草db2
                   sql = string.Format(@"SELECT A.* ,SUBSTR(A.ORDER_ID,3,12) AS ORDERID,B.BRAND_N AS BRANDCODE FROM V_WMS_SORT_ORDER_DETAIL A
