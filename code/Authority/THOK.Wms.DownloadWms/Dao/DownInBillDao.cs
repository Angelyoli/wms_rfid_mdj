...
namespace THOK.WMS.DownloadWms.Dao
{
    public class DownInBillDao : BaseDao
    {
        ...
        {
            string sql = string.Format("SELECT ORDER_ID AS BILLNO,ORDER_DATE AS BILLDATE,ORDER_TYPE AS BILLTYPE,QUANTITY_SUM AS QUANTITY_SUM,DETAIL_NUM AS DETAIL_NUM FROM V_WMS_IN_ORDER WHERE ORDER_ID NOT IN ({0}) AND QUANTITY_SUM>0", billNo);           
            return this.ExecuteQuery(sql).Tables[0];
        }
        ...
        public DataTable GetInBillDetailByBillNo(string inBillNo)
        {
            string sql = @"SELECT ORDER_DETAIL_ID AS ID,ORDER_ID AS BILLNO,BRAND_CODE AS PRODUCTCODE,BRAND_NAME AS PRODUCT_NAME,QUANTITY AS QUANTITY
                           FROM V_WMS_IN_ORDER_DETAIL WHERE ORDER_ID='{0}' AND QUANTITY>0";
